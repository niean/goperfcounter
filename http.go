...
func configProcRoutes() {
	...
	http.HandleFunc("/pfc/proc/metric/", func(w http.ResponseWriter, r *http.Request) {
		if !isLocalReq(r.RemoteAddr) {
			RenderJson(w, "no privilege")
			return
		}
		urlParam := r.URL.Path[len("/pfc/proc/metric/"):]
		...
		if args[1] == "json" {
			RenderJson(w, rawMetric(args[0]))
			return
		}
		if args[1] == "falcon" {
			RenderJson(w, falconMetric(args[0]))
			return
		}
	})
}
